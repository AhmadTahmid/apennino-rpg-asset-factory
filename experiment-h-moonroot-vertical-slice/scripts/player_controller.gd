...
func command_move(target: Vector2, timeout_seconds := 4.0) -> void:
...
func teleport_to(position_px: Vector2, row: int = ROW_DOWN) -> void:
	reset_for_test(position_px)
	facing_row = clampi(row, ROW_DOWN, ROW_UP)
	_apply_frame(IDLE_COLUMN, facing_row, "node_transition")
...
func _apply_frame(column: int, row: int, reason: String) -> void:
	...
	animation_trace.append({"physics_frame": Engine.get_physics_frames(), "column": column, "row": row, "reason": reason})
